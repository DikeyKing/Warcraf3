...
#import "WAR3OrcHeroViewController.h"
#import "WAR3OrcUnitViewController.h"
#import "WAR3OrcArchitectViewController.h"

@interface WAR3OrcViewController ()

@end

@implementation WAR3OrcViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 ) {
        WAR3OrcHeroViewController *ohvc= [WAR3OrcHeroViewController new];
        ohvc.selectedRow = indexPath.row;
        [self.navigationController pushViewController:ohvc animated:YES];
    }
    if (indexPath.section == 1 ) {
        WAR3OrcUnitViewController *ouvc = [WAR3OrcUnitViewController new];
    ...
    if (indexPath.section == 2 ) {
        WAR3OrcArchitectViewController *oavc = [WAR3OrcArchitectViewController new];
        oavc.selectedRow = indexPath.row;
...
- (void)viewDidLoad
{
    ...
    WAR3OrcHero *orcHero = [[WAR3OrcHero alloc]init];
    [orcHero getDeserializedArray];
    
    NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
    
    [dict setObject:@"兽族英雄" forKey:@"groupname"];
    
    //Arr1 初始化以及个数初始化
    NSMutableArray *arr1 = [[NSMutableArray alloc]initWithCapacity:[orcHero.deserializedArray count]];
    
    //将hero array中的所有名字加入到 arr1 中
    for (int i = 0; i<[orcHero.deserializedArray count]; i++) {
        NSDictionary *dictionary  = orcHero.deserializedArray[i];
    ...
    WAR3OrcUnit *orcUnit= [[WAR3OrcUnit alloc]init];
    [orcUnit getDeserializedArray];
    dict = [[NSMutableDictionary alloc]initWithCapacity : [orcUnit.deserializedArray count]];
    [dict setObject:@"兽族单位" forKey:@"groupname"];
    NSMutableArray *arr2 = [[NSMutableArray alloc]initWithCapacity:[orcUnit.deserializedArray count]];
    
    for (int i = 0 ; i< [orcUnit.deserializedArray count]; i++) {
        NSDictionary *dictionary = orcUnit.deserializedArray[i];
        NSString *name = [dictionary objectForKey:@"name"];
        [arr2 addObject:name];
    }
    ...
    WAR3OrcArchitect *orcArchitect = [[WAR3OrcArchitect alloc]init];
    [orcArchitect getDeserializedArray];
    dict = [[NSMutableDictionary alloc]initWithCapacity:[orcArchitect.deserializedArray count]];
    [dict setObject:@"兽族建筑" forKey:@"groupname"];
    NSMutableArray *arr3 = [[NSMutableArray alloc]initWithCapacity:[orcArchitect.deserializedArray count]];
    for (int i = 0; i<[orcArchitect.deserializedArray count]; i++) {
        NSDictionary *dictionary = orcArchitect.deserializedArray[i];
        NSString *name  = [dictionary objectForKey:@"name"];
        [arr3 addObject:name];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [orcArchitect deserializedArray];
    
    if (indexPath.section ==0 ) {
        NSArray *pathArray = [NSArray arrayWithObjects:@"blademaster.gif",@"farseer.gif",@"taurenchieftain.gif",@"shadowhunter.gif", nil];
    ...
    if (indexPath.section ==1 ) {
        NSMutableArray *imageArray =[[NSMutableArray alloc]initWithCapacity:[orcUnit.deserializedArray count]];
        for (int i = 0 ; i< [orcUnit.deserializedArray count]; i++) {
            NSDictionary *dictionary = orcUnit.deserializedArray[i];
    ...
    if (indexPath.section ==2){
        NSMutableArray *imageArray = [[NSMutableArray alloc]initWithCapacity:[orcArchitect.deserializedArray count]];
        for (int i = 0; i<[orcArchitect.deserializedArray count]; i++) {
            NSDictionary *dictionary = orcArchitect.deserializedArray[i];
            NSString *imageName= [dictionary objectForKey:@"imageName"];
            [imageArray addObject:imageName];
